 ...
 * 2. URL 参数   ：?debug=bar / ?debug=1 / ?debug=0
 * 3. 环境变量   ：VITE_DEBUG / DEBUG / NODE_DEBUG（Node 与 SSR 场景）
 *
 * 并补齐包本身不支持的「通配符 / 短名」匹配，例如 `note-bar:*`、`bar`。
...
export const DEBUG_NAMESPACES = ['note-bar:bar', 'note-bar:demo'] as const;
 ...
 * 前缀（'note-bar:*'）、短名（'bar' 命中 'note-bar:bar'）、通配（'*bar*'）。
 */
...
/**
 * 逗号分隔的批量开关，语义同 debug 包：`'bar,-note-bar:demo'`。
...
export function describeDebug(): string {
  ...
  return `note-bar debug: ${on.length ? on.join(', ') : '（全部关闭）'}`;
}
...
export const logBar = createLogger('note-bar:bar');
export const logDemo = createLogger('note-bar:demo');
